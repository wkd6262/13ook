...
<details>
  ...
  | ... | 페이지 |기능 구현|
  |---|---|---|
  | 장원준 |- 메인 페이지<br> - 검색 페이지<br> - UI 디자인 <br> |- 외부 API 연결 :알라딘API(검색기능,베스트셀러, 신간목록) <br> - 파일 병합|
  | 김민재 |- 로그인 페이지<br> - 책방 페이지 |- 외부 API 연결 : 카카오 로그인API|
  | 권지민 |- 피드 페이지<br> - 내서재 페이지 |- 회원가입 및 로그인 mongoDB 연결 <br> - 클라우드타입 배포 |
